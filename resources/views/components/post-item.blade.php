<div class="flex bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 mb-8">
    <div class="p-5 flex-1">
        ...
        <div class="text-sm text-gray-400 flex gap-4 mt-2">
            <div class="">
                published by
                <a href="{{ route('profile.show', $post->user->username) }}" class="text-gray-200 hover:underline"><strong>{{ $post->user->username }}</strong></a>
                at
    ...
    <a href="{{ route('post.show', ['username'=>$post->user->username, 'post'=>$post->slug]) }}">
        {{-- <img class="rounded-t-lg w-48 h-full max-h-64 object-cover" src="{{Storage::url($post->image)}}" alt="{{$post->title}}" /> --}}
        <img class="rounded-t-lg w-48 h-full max-h-64 object-cover" src="{{ $post->imageUrl() }}" alt="{{$post->title}}" />
    </a>
</div>
